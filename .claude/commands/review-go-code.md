...
**审查准则（必须严格遵守）：**
请结合上面的静态检查结果，并根据我们项目根目录下的 constitution.md 文件定义的开发宪法，对代码进行审查：
1. 第一条：简单性原则。代码是否存在不必要的抽象？是否优先使用了标准库？
2. 第三条：明确性原则。错误处理是否都使用了 fmt.Errorf("...: %w", err) 进行包装？是否有隐式丢弃的错误？
...
- **待改进项（按优先级排序）：**
    - [高优先级]: 必须修改的问题（违反"宪法"不可协商条款或 go vet 报告的错误）。
    - [中优先级]: 强烈建议修改的问题（设计模式可以优化）。
    - [低优先级]: 代码风格或可读性建议。
